...
import { FileText, Plus, PenTool, Trash2, Edit } from 'lucide-react';
import { toast } from 'sonner';

interface Props {
  templates: SignatureTemplate[];
  onCreateTemplate: (name: string, description?: string, documentContent?: string) => any;
...
export function SignatureTemplatesList({ templates, onCreateTemplate, onDeleteTemplate, onSelectTemplate, onUpdateTemplate }: Props) {
  ...
  const handleCreate = async () => {
    ...
    await onCreateTemplate(newName.trim(), newDesc.trim() || undefined, newContent.trim() || undefined);
    setShowCreate(false);
    setNewName('');
    setNewDesc('');
    setNewContent('');
            ...
            <Card key={tpl.id} className="border border-border/50 hover:border-primary/30 transition-colors cursor-pointer" onClick={() => onSelectTemplate(tpl)}>
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground">
                  ...
                  {tpl.documentContent && <span className="text-emerald-600">✓ Contenu</span>}
                </div>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Contenu du document (optionnel)</Label>
              <Textarea
                value={newContent}
                onChange={e => setNewContent(e.target.value)}
                rows={6}
                placeholder="Collez ici le texte de votre mandat de gestion. Vous pourrez ensuite ajouter des variables dynamiques dans l'éditeur."
                className="font-serif text-sm"
              />
              <p className="text-[10px] text-muted-foreground">
                Vous pourrez modifier le contenu et ajouter des variables dynamiques après la création.
              </p>
            </div>
          </div>
          <DialogFooter>
