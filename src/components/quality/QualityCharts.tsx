...
export function QualityCharts({
  ...
  const formattedReworkTrend = reworkTrend.slice(-14).map(point => ({
    ...point,
    formattedDate: format(parseISO(point.date), 'dd/MM', { locale: fr }),
  }));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-medium">Taux de reprise</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={formattedReworkTrend}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                <XAxis 
                  dataKey="formattedDate" 
                  tick={{ fontSize: 12 }}
