...
export function OperationsFlowSection() {
  ...
  return (
    <section ref={ref} className="relative py-28 lg:py-40" id="operations">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {/* Grid of operation cards — single pixel border grid */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border/30 rounded-2xl overflow-hidden">
          {operations.map((op, i) => (
            ...
            >
              <p className="text-[13px] font-medium text-foreground tracking-wide mb-5">
                {op.label}
              </p>
              ...
              <div className="space-y-1.5">
                {op.lines.map((line, j) => (
                  <motion.p
                    ...
                    transition={{ duration: 0.3, delay: 0.3 + i * 0.08 + j * 0.08 }}
                  >
                    {line}
                  </motion.p>
                ))}
