...
const generateMockRules = (): AlertRule[] => {
  ...
  return [
    {
      id: 'rule-1',
      name: "Logement sous la moyenne d'occupation du portefeuille",
      description: "Se déclenche par logement quand son taux d'occupation est inférieur d'au moins 15 % à la moyenne des autres biens du portefeuille sur 30 jours.",
      category: 'occupancy',
      enabled: true,
      priority: 'high',
      ...
      notificationTemplate: {
        title: "Logement sous la moyenne d'occupation",
        message: "{{property_name}} affiche un taux d'occupation de {{metric_value}} sur les {{period}}, soit {{delta}} en dessous de la moyenne du reste du portefeuille ({{baseline_value}}).",
        suggestedActions: [
    ...
    {
      id: 'rule-portfolio-occupancy',
      name: "Taux d'occupation moyen du portefeuille sous le seuil",
      description: "Alerte globale (non rattachée à un logement) qui se déclenche dès que le taux d'occupation moyen de l'ensemble du portefeuille passe sous l'objectif fixé.",
      category: 'occupancy',
      enabled: true,
      priority: 'high',
      metric: 'occupancy_rate',
      baseline: 'fixed_target',
      thresholdType: 'absolute',
      thresholdValue: 65,
      thresholdOperator: 'below',
      severityThresholds: { warning: 60, critical: 50 },
      timeWindow: '30d',
      timeDirection: 'past',
      triggerFrequency: 'daily',
      scope: 'all',
      notificationTemplate: {
        title: "Occupation portefeuille sous l'objectif",
        message: "Le taux d'occupation moyen du portefeuille est de {{metric_value}} sur les {{period}}, sous l'objectif fixé de {{baseline_value}}. Action recommandée : revoir la grille tarifaire ou les durées minimum.",
        suggestedActions: [
          { id: 'po1', label: 'Ouvrir le calendrier tarifaire', actionType: 'open_pricing' },
          { id: 'po2', label: 'Gérer les règles', actionType: 'open_rules' },
        ],
      },
      cooldownDays: 3,
      autoArchiveDays: 14,
