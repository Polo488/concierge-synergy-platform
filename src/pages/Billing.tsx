...
import { 
  ...
  Trash, CheckCircle2 // Ajout des icônes nécessaires
} from 'lucide-react';
...
// Add these imports
import { ImportLogs } from '@/components/billing/ImportLogs';
...
const Billing = () => {
  // Add the bookingSync hook
  const bookingSync = useBookingSync();
  
  useEffect(() => {
    document.title = 'Facturation - Concierge Synergy Platform';
    
    // Try auto-authentication on component mount
    bookingSync.autoAuthenticateMutation.mutate();
  }, []);
    ...
    <div className="space-y-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="import" className="space-y-6">
          <div className="grid md:grid-cols-3 gap-6">
            ...
            >
              <div className="space-y-4">
                ...
                <Button 
                  className="w-full" 
                  onClick={() => setSmilyImportOpen(true)}
                  disabled={bookingSync.importQuery.isLoading}
                >
                  <RefreshCw className={`h-4 w-4 mr-2 ${bookingSync.importQuery.isLoading ? 'animate-spin' : ''}`} />
                  {bookingSync.importQuery.isLoading ? 'Importation en cours...' : 'Importer depuis SMILY'}
                </Button>
                
                {bookingSync.isAuthenticated ? (
                  <p className="text-xs text-green-600 flex items-center">
                    <CheckCircle className="h-3 w-3 mr-1" /> Connecté à SMILY
                  </p>
                ) : (
                  <p className="text-xs text-amber-600 flex items-center">
                    <AlertTriangle className="h-3 w-3 mr-1" /> Non connecté
                  </p>
                )}
              </div>
            </DashboardCard>
          ...
          {/* Import Logs */}
          <DashboardCard title="Logs et résultats d'import">
            <div className="grid gap-8 md:grid-cols-2">
              <ImportLogs logs={bookingSync.logs} onClearLogs={bookingSync.clearLogs} />
              
              <div className="space-y-2">
                <h3 className="font-medium">Imports récents</h3>
                <Table>
                  <TableHeader>
                    <TableRow>
